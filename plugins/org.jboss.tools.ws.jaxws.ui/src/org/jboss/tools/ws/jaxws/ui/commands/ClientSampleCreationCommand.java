...
public class ClientSampleCreationCommand extends AbstractDataModelOperation {
	...
	public IStatus execute(IProgressMonitor monitor, IAdaptable info)
		...
		List<ICompilationUnit> clientUnits = JBossWSCreationUtils
				.findJavaUnitsByAnnotation(
						project,
						JBossJAXWSCoreMessages.WebserviceClient_Annotation,
	...
	private IStatus createImplClass(String packageName, IJavaProject project,
		...
		ICompilationUnit clientCls = createJavaClass(packageName
				+ JBossJAXWSCoreMessages.Client_Sample_Package_Name,
				JBossJAXWSCoreMessages.Client_Sample_Class_Name, false,
				null, project);
		if (clientCls == null) {
			return StatusUtils
					.errorStatus(JBossJAXWSCoreMessages.Error_Create_Client_Sample);
		}
		...
		try {
			...
			JBossJAXWSUIPlugin.getDefault().logError(e1);
			return StatusUtils
					.errorStatus(JBossJAXWSCoreMessages.Error_Create_Client_Sample);
		}
		...
		sb.append("        System.out.println(\"").append( //$NON-NLS-1$
				JBossJAXWSCoreMessages.Client_Sample_Run_Over)
				.append("\");"); //$NON-NLS-1$
		sb.append(LINE_SEPARATOR);
		sb.append("}"); //$NON-NLS-1$
		try {
			...
			for (MethodDeclaration method : methodDec) {
				if (method.modifiers().get(0) instanceof NormalAnnotation) {
					...
					if (anno.getTypeName().getFullyQualifiedName()
							.equals(JBossJAXWSCoreMessages.WebEndpoint)) {
